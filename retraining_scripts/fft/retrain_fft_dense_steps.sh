#!/bin/bash
#SBATCH --job-name=fft_dense_steps
#SBATCH --output=/lustre/fswork/projects/rech/oag/unz84ar/programs/open-instruct/logs/retrain_fft_dense_%j.out
#SBATCH --error=/lustre/fswork/projects/rech/oag/unz84ar/programs/open-instruct/logs/retrain_fft_dense_%j.err
...
#SBATCH --time=06:00:00         # ~1000 steps, much shorter than full run
...
# Reproduce steps 0-1000 with the same data order as the original training
# (same seed=42 default, same dataset, same batch geometry → identical shuffle)
# Checkpoints every 50 steps instead of 1000, stopped at step 1000.
srun uv run --no-sync --offline accelerate launch \
    ...
    open_instruct/finetune.py \
      ...
      --learning_rate 5e-6 \
      --num_train_epochs 2 \
      --output_dir /lustre/fswork/projects/rech/oag/unz84ar/programs/open-instruct/retrain_fft_dense_steps \
      --logging_steps 50 \
