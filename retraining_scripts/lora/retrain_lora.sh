...
#SBATCH --cpus-per-task=24      # IDRIS recommended setting for H100
...
module load pytorch-gpu/py3/2.6.0  # Or 2.4.0 as per your slide
...
# PHB : PCIe Host Bridge to allow GPUs to communicate with each other without CPUs
export NCCL_NET_GDR_LEVEL=PHB
# HCA : Host Channel Adapter (physical InfiniBand network card)
export NCCL_IB_HCA=$(python3 -c "import os; print(','.join(['mlx5_%d' % i for i in range(4)]))")
# Socket part = initial handshake between machines. ^ means ignore
...
srun uv run --no-sync --offline accelerate launch \
    ...
    open_instruct/finetune.py \
      ...
      --resume_from_checkpoint /lustre/fswork/projects/rech/oag/unz84ar/programs/open-instruct/retrain_lora/step_1000/ \
      ...
      --learning_rate 5e-6 \
      --num_train_epochs 2 \
      --output_dir /lustre/fswork/projects/rech/oag/unz84ar/programs/open-instruct/retrain_lora \
      --logging_steps 50 \
